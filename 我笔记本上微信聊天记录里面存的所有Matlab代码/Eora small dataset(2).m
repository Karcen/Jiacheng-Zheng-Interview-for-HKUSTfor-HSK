...
N=4; % number of srctors
GN=G*N;
fd_components=1;
% in full Eora, fd_components=6;

    z = xlsread('C:\Users\Jiacheng Zheng\Desktop\books\TCBA_Karcen','TCBA Example',"C4:N15");
    fd = xlsread('C:\Users\Jiacheng Zheng\Desktop\books\TCBA_Karcen','TCBA Example',"R4:T15");
    for i = 1:G
    ...
    e_b_intmd_bysector_both= z - z.*id_blockeye;
    for i=1:G
...
%% Term 7
 ...
 % term7 is the 7rd term in Koopman eq 76s
    %--------------
    for s=1:G
        for r=1:G
            B_cell_t7{s,r}=B_cell{s,r};
            B_cell_t7{s,s}=zeros(N);    %sets diagonal terms to zero in the B matrix
            y_cell_t7{s,r}=y_cell{s,r};
            y_cell_t7{s,s}=zeros(N,1);  %sets diagonal terms to zero in the y matrix
        end
    end
    for s=1:G
        for r=1:G
            for q=1:G
                B_term7{s,r,q}=zeros(N);
                B_term7{s,r,s}=B_cell_t7{s,r};     %leaves rows set to zero, and puts above modified B_cell when s=t
                
                y_term7{s,r,q}=y_cell_t7{s,r};     %puts above modified y_cell everywhere
                y_term7{s,r,r}=zeros(N,1);         %then sets columns to zero when r=t
                y_term7{s,r,s}=zeros(N,1);         %then sets columns to zero when r=s
            end
        end
    end
    B_term7_m=cell2mat(B_term7);
    y_term7_m=cell2mat(y_term7);

    % clear B_term7;
    clear y_term7;
    clear V_term7
    for q=1:G
        term7_1m(:,:,q)=B_term7_m(:,:,q)*y_term7_m(:,:,q);
    end

    term7_1m_reshape = reshape(term7_1m,[GN,G*G]);
    % clear term7_1m B_term7_m y_term7_m;
    clear term7_1m y_term7_m;
    % term7_b stands for bilateral
    term7_b_temp = V*term7_1m_reshape;
    for i=1:G
        term7_b(i,:)=term7_b_temp(i,((i-1)*G+1):G*i);
    end   
    sum_term7_1m = sum(term7_1m_reshape,2);
    term7 = V*sum_term7_1m;
    term7_pct_export=term7./e';
    sum_terms7=sum_terms3456+term7_pct_export;
    disp('Done with term 7')
