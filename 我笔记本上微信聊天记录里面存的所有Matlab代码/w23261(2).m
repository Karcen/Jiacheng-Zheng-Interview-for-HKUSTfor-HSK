...
nfd=6;  % number of finaldemand
    ...
    % 创建文件路径
    filepath = 'E:\China_vs_India\PAK\Wang\2009\2009_SML.csv'
    filename = 'E:\China_vs_India\PAK\GVC\2009_PAK_GVC_Length.xlsx';
    output_filename = 'E:\China_vs_India\PAK\GVC\GVC Length\Country Level\2009_country_level_data.xlsx';
     G = xlsread(filepath);
...
result2.PL_D_F(isnan(result2.PL_D_F) | isinf(result2.PL_D_F)) = 0;

result2.PL_D_B = reshape(result.PL_D_B', 45, 77);
result2.PL_D_B(isnan(result2.PL_D_B) | isinf(result2.PL_D_B)) = 0;

result2.PL_RT_F = reshape(result.PL_RT_F, 45, 77);
result2.PL_RT_F(isnan(result2.PL_RT_F) | isinf(result2.PL_RT_F)) = 0;

result2.PL_RT_B = reshape(result.PL_RT_B', 45, 77);
result2.PL_RT_B(isnan(result2.PL_RT_B) | isinf(result2.PL_RT_B)) = 0;

result2.PL_GVC_F = reshape(result.PL_GVC_F, 45, 77);
result2.PL_GVC_F(isnan(result2.PL_GVC_F) | isinf(result2.PL_GVC_F)) = 0;

result2.PL_GVC_B = reshape(result.PL_GVC_B', 45, 77);
result2.PL_GVC_B(isnan(result2.PL_GVC_B) | isinf(result2.PL_GVC_B)) = 0;

result2.PL_GVC_S_F = reshape(result.PL_GVC_S_F, 45, 77);
result2.PL_GVC_S_F(isnan(result2.PL_GVC_S_F) | isinf(result2.PL_GVC_S_F)) = 0;

result2.PL_GVC_S_B = reshape(result.PL_GVC_S_B', 45, 77);
result2.PL_GVC_S_B(isnan(result2.PL_GVC_S_B) | isinf(result2.PL_GVC_S_B)) = 0;

result2.PL_GVC_C_F = reshape(result.PL_GVC_C_F, 45, 77);
result2.PL_GVC_C_F(isnan(result2.PL_GVC_C_F) | isinf(result2.PL_GVC_C_F)) = 0;

result2.PL_GVC_C_B = reshape(result.PL_GVC_C_B', 45, 77);
result2.PL_GVC_C_B(isnan(result2.PL_GVC_C_B) | isinf(result2.PL_GVC_C_B)) = 0;
...
country.PL_D_FT = country.PL_D_F';
country.PL_D_BT = country.PL_D_B';
country.PL_RT_FT = country.PL_RT_F';
country.PL_RT_BT = country.PL_RT_B';
country.PL_GVC_FT = country.PL_GVC_F';
country.PL_GVC_BT = country.PL_GVC_B';
country.PL_GVC_S_FT = country.PL_GVC_S_F';
country.PL_GVC_S_BT = country.PL_GVC_S_B';
country.PL_GVC_C_FT = country.PL_GVC_C_F';
country.PL_GVC_C_BT = country.PL_GVC_C_B';


country_level=[country.PL_D_FT country.PL_D_BT country.PL_RT_FT country.PL_RT_BT country.PL_GVC_FT country.PL_GVC_BT country.PL_GVC_S_FT country.PL_GVC_S_BT country.PL_GVC_C_FT country.PL_GVC_C_BT];

%% Output
% Define the headers without the 'country.' prefix
headers = {'PL_D_F', 'PL_D_B', 'PL_RT_F', 'PL_RT_B', 'PL_GVC_F', 'PL_GVC_B', 'PL_GVC_S_F', 'PL_GVC_S_B','PL_GVC_C_F', 'PL_GVC_C_B'};

% Combine headers with data
data_to_write = [headers; num2cell(country_level)];

% Specify the output Excel file name


% Write the data to an Excel file
writecell(data_to_write, output_filename);
...
% 提取表头
headersForward = {'PL_D_F', 'PL_RT_F', 'PL_GVC_F', 'PL_GVC_S_F', 'PL_GVC_C_F'};
headersBackward = {'PL_D_B', 'PL_RT_B', 'PL_GVC_B', 'PL_GVC_S_B', 'PL_GVC_C_B'};

% 定义Excel文件名和工作表名称

sheetForward = 'Forward Linkages';
sheetBackward = 'Backward Linkages';

% 写入 Forward 数据
xlswrite(filename, headersForward, sheetForward, 'A1');
xlswrite(filename, PAK.All.Forward, sheetForward, 'A2');

% 写入 Backward 数据
xlswrite(filename, headersBackward, sheetBackward, 'A1');
xlswrite(filename, PAK.All.Backward, sheetBackward, 'A2');
