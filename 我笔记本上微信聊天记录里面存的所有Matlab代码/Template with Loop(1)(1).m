...
s.sector.GVCsParticipation=(DVAX2+REX+REF)./s.sector.DVA.Tot;
...
% csvwrite('E:\China_vs_India\PAK\Wang\1996\PAK.csv',PAK)  
        ...
        output_filepath2 = sprintf('E:\\China_vs_India\\PAK\\Wang\\%d\\%d_GVCs_Particiantion.csv', year, year);
        outputFilePath = sprintf('E:\\China_vs_India\\PAK\\Wang\\%d\\allData_%d.mat', year, year);
        % 打开文件写入模式
        ...
        dlmwrite(output_filepath2, s.country.GVCsParticipationM, '-append');
        save(outputFilePath);
    else
        fprintf('文件 %s 不存在。\n', filepath);
    end
