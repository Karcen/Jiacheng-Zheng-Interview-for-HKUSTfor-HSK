...
%% Block Creation and Row Sum Calculation
% Create blocks from the z matrix (4x4 sub-matrices), calculate row sums
z_block = cell(2*G,2*G); % Cell array for blocks
z_block_row_sum = cell(2*G,2*G); % Cell array for row sums of blocks
for i = 1:2*G
for j = 1:2*G
z_block{i,j} = z((i-1)*S+1:i*S, (j-1)*S+1:j*S); % Extract 4x4 blocks
z_block_row_sum{i,j} = sum(z_block{i,j}, 2); % Row sum of each block
end
end
z_block_row_sumM = cell2mat(z_block_row_sum); % Convert cell array to matrix
%%
% Extract Diagonal Blocks and Create Domestic and Export Matrices of Intermediate trade
result = zeros(size(z_block_row_sumM)); % Initialize result matrix (for domestic)
block_size_rows = N; % Number of rows in each block
block_size_cols = 2; % Number of columns in each block
num_blocks = floor(size(z_block_row_sumM, 1) / block_size_rows); % Number of row blocks
% Copy diagonal blocks to result matrix (Domestic matrix)
for i = 1:num_blocks
row_start = (i - 1) * block_size_rows + 1;
row_end = i * block_size_rows;
col_start = (i - 1) * block_size_cols + 1;
col_end = min(i * block_size_cols, size(z_block_row_sumM, 2)); % Stay within bounds
% Copy the diagonal block to the result matrix
result(row_start:row_end, col_start:col_end) = z_block_row_sumM(row_start:row_end, col_start:col_end);
end
Z_Domestic_DF = result; % Domestic component
Z_Exports = z_block_row_sumM - Z_Domestic_DF; % Export component

%%
%for domestic transaction
for i=1:G
    AMNE.Z_Domestic_D(41*i-40:41*i,:)=Z_Domestic_DF(82*i-81:82*i-41,:); % extracting 41 D and F by rows
    AMNE.Z_Domestic_F(41*i-40:41*i,:)=Z_Domestic_DF(82*i-40:82*i,:); % extracting 41 D and F by rows
end

for i=1:G
    AMNE.z_DD(:,i)=AMNE.Z_Domestic_D(:,2*i-1); % extracting odd(domestic) by columns
    AMNE.z_DF(:,i)=AMNE.Z_Domestic_D(:,2*i); % extracting even(foreign) by columns
    AMNE.z_FD(:,i)=AMNE.Z_Domestic_F(:,2*i-1); % extracting odd(domestic) by columns
    AMNE.z_FF(:,i)=AMNE.Z_Domestic_F(:,2*i); % extracting even(foreign) by columns
end
     
AMNE.z_DDs=sum(AMNE.z_DD,2);
AMNE.z_DFs=sum(AMNE.z_DF,2);
AMNE.z_FDs=sum(AMNE.z_FD,2);
AMNE.z_FFs=sum(AMNE.z_FF,2);

AMNE.z_total=AMNE.z_DDs+AMNE.z_DFs+AMNE.z_FDs+AMNE.z_FFs;

sum1=sum(AMNE.z_total);
sum2=sum(sum(Z_Domestic_DF));


%%
%for exports transactions
for i=1:G
    AMNE.Z_Exports_D(41*i-40:41*i,:)=Z_Exports (82*i-81:82*i-41,:); % extracting 41 D and F by rows
    AMNE.Z_Exports_F(41*i-40:41*i,:)=Z_Exports (82*i-40:82*i,:); % extracting 41 D and F by rows
end

for i=1:G
    AMNE.zexports_DD(:,i)=AMNE.Z_Exports_D(:,2*i-1); % extracting odd(domestic) by columns
    AMNE.zexports_DF(:,i)=AMNE.Z_Exports_D(:,2*i); % extracting even(foreign) by columns
    AMNE.zexports_FD(:,i)=AMNE.Z_Exports_F(:,2*i-1); % extracting odd(domestic) by columns
    AMNE.zexports_FF(:,i)=AMNE.Z_Exports_F(:,2*i); % extracting even(foreign) by columns
end
     
AMNE.zexports_DDs=sum( AMNE.zexports_DD,2);
AMNE.zexports_DFs=sum( AMNE.zexports_DF,2);
AMNE.zexports_FDs=sum( AMNE.zexports_FD,2);
AMNE.zexports_FFs=sum( AMNE.zexports_FF,2);

AMNE.zexports_total=AMNE.zexports_DDs+AMNE.zexports_DFs+AMNE.zexports_FDs+AMNE.zexports_FFs;

sum3=sum(AMNE.zexports_total);
sum4=sum(sum(Z_Exports));
...
%% Z_exports
for i=1:G
    AMNE.Z_Exports_D(41*i-40:41*i,:)=Z_Exports(82*i-81:82*i-41,:);
    AMNE.Z_Exports_F(41*i-40:41*i,:)=Z_Exports(82*i-40:82*i,:);
    
    AMNE.Z_Exports_DD(:,i)=AMNE.Z_Exports_D(:,2*i-1);
    AMNE.Z_Exports_FF(:,i)=AMNE.Z_Exports_D(:,2*i);
end

AMNE.Z_Exports_DDs=sum(AMNE.Z_Exports_DD,2);
AMNE.Z_Exports_FFs=sum(AMNE.Z_Exports_FF,2);

AMNE.Z_Exports_total=AMNE.Z_Exports_DDs+AMNE.Z_Exports_FFs;

%% Extract GFCF for Every Country
for i=1:G
    GFCF(:,i)=FD2(:,6*i-2);
end
%%
for i=1:G
    AMNE.GFCF_D(41*i-40:41*i,:)=GFCF(82*i-81:82*i-41,:);
    AMNE.GFCF_F(41*i-40:41*i,:)=GFCF(82*i-40:82*i,:);
end
...
for i=1:G
    AMNE.X_D(41*i-40:41*i,:)=Xt(82*i-81:82*i-41,:);
    AMNE.X_F(41*i-40:41*i,:)=Xt(82*i-40:82*i,:);
end

VAD_t=VaD';
for i=1:G
    AMNE.VAD_D(41*i-40:41*i,:)=VAD_t(82*i-81:82*i-41,:);
    AMNE.VAD_F(41*i-40:41*i,:)=VAD_t(82*i-40:82*i,:);
end

% for i=1:G
%     AMNE.z_D{:,i}=z(:,82*i-81:82*i-41);
%     AMNE.z_F{:,i}=z(:,82*i-40:82*i);
% end

% AMNE.z_DM=cell2mat(AMNE.z_D);
% AMNE.z_FM=cell2mat(AMNE.z_F);
...
TTerm1.GDP=sum(Term1.total,2);
TTerm1.FD=sum(Term1.total,1);

% Y_L_D pure domestic production
...
Results.NGVC.PDP=Term1.T1; % sum them to one column
Results.NGVC.Trad_Trade=Term1.T2; % sum them to one column
%𝐺𝑉𝐶𝑠 𝑎𝑐𝑡𝑖𝑣𝑖𝑡𝑖𝑒
% Term1.T3=diag(V_D)*LM*A_E*B*diag(Y_D); 
Results.GVC.Trade_GVC=Term1.T3; % FIE is zero, every even block is empty. 
Results.GVC.Trade_GVC_st.simple=Term1.T31.T11;
Results.GVC.Trade_GVC_st.complex=Term1.T31.T12;

%% Trade-related GVCs Term3

AMNE.Eq6.T3.main=Results.GVC.Trade_GVC;
for i=1:G
    AMNE.Eq6.T3.T3_D(41*i-40:41*i,:)=Results.GVC.Trade_GVC(82*i-81:82*i-41,:);
    % AMNE.Eq6.T3.T3_F(41*i-40:41*i,:)=Results.GVC.Trade_GVC(82*i-40:82*i,:);
    
    % AMNE.Eq6.T3_DD(:,i)=AMNE.Eq6.T3_D(:,2*i-1);
    % AMNE.Eq6.T3_FF(:,i)=AMNE.Eq6.T3_D(:,2*i);
end

for i=1:G
    AMNE.Eq6.T3.T3_Fin_D1(:,41*i-40:41*i)=AMNE.Eq6.T3.T3_D(:,82*i-81:82*i-41);
    % AMNE.Eq6.T3.T3_F1(:,41*i-40:41*i)=AMNE.Eq6.T3.T3_F(:,82*i-40:82*i);
...
for i=1:G
    AMNE.Eq6.T3.T3_Fin_D1_SL(:,i)=sum(AMNE.Eq6.T3.T3_Fin_D1(:,41*i-40:41*i),2); % make 3147 to 77, add every 41 cells
end

for i=1:G
     AMNE.Eq6.T3.T3_Fin_D1_CL(i,:)=sum(AMNE.Eq6.T3.T3_Fin_D1_SL(41*i-40:41*i,:),1);
end
...
AMNE.Eq6.T4.T4FDI_GVC.V_doe_Y_FIE_L=sum(Results.GVC.FDI_GVC.V_doe_Y_FIE_L,2);

AMNE.Eq6.T5.T5FDI_GVC.V_doe_Y_FIE_E=sum(Results.GVC.FDI_GVC.V_doe_Y_FIE_E,2);
%  Extract the odd rows
for i=1:G
    AMNE.Eq6.T4.T4FDI_GVC.V_doe_Y_FIE_L_SL(41*i-40:41*i,:)=AMNE.Eq6.T4.T4FDI_GVC.V_doe_Y_FIE_L(82*i-81:82*i-41,:);
    AMNE.Eq6.T5.T5FDI_GVC.V_doe_Y_FIE_E_SL(41*i-40:41*i,:)=AMNE.Eq6.T5.T5FDI_GVC.V_doe_Y_FIE_E(82*i-81:82*i-41,:);
    % AMNE.Eq6.T3.T3_F(41*i-40:41*i,:)=Results.GVC.Trade_GVC(82*i-40:82*i,:);
...
AMNE.Eq6.T4.T4FDI_GVC.V_doe_Y_FIE_L_SL=reshape(AMNE.Eq6.T4.T4FDI_GVC.V_doe_Y_FIE_L_SL,41,77);
AMNE.Eq6.T4.T4FDI_GVC.V_doe_Y_FIE_L_CL=sum(AMNE.Eq6.T4.T4FDI_GVC.V_doe_Y_FIE_L_SL,1)';

AMNE.Eq6.T5.T5FDI_GVC.V_doe_Y_FIE_E_SL=reshape(AMNE.Eq6.T5.T5FDI_GVC.V_doe_Y_FIE_E_SL,41,77);
AMNE.Eq6.T5.T5FDI_GVC.V_doe_Y_FIE_E_CL=sum(AMNE.Eq6.T5.T5FDI_GVC.V_doe_Y_FIE_E_SL,1)';
...
AMNE.Eq6.T6.T6GVC.FDI_GVC.V_fie_Y_DOE_L=sum(Results.GVC.FDI_GVC.V_fie_Y_DOE_L,2);
AMNE.Eq6.T6.T6GVC.FDI_GVC.V_fie_Y_DOE_L=reshape(AMNE.Eq6.T6.T6GVC.FDI_GVC.V_fie_Y_DOE_L,41,154);

for i=1:G
    AMNE.Eq6.T6.T6GVC.FDI_GVC.V_fie_Y_DOE_SL(:,i)=AMNE.Eq6.T6.T6GVC.FDI_GVC.V_fie_Y_DOE_L(:,2*i);
end

AMNE.Eq6.T6.T6GVC.FDI_GVC.V_fie_Y_DOE_CL=sum(AMNE.Eq6.T6.T6GVC.FDI_GVC.V_fie_Y_DOE_SL,1)';

Results.GVC.FDI_GVC.V_fie_Y_DOE_E=Term1.T7;
AMNE.Eq6.T7.T7GVC.FDI_GVC.V_fie_Y_DOE_E=sum(Results.GVC.FDI_GVC.V_fie_Y_DOE_E,2);
AMNE.Eq6.T7.T7GVC.FDI_GVC.V_fie_Y_DOE_E=reshape(AMNE.Eq6.T7.T7GVC.FDI_GVC.V_fie_Y_DOE_E,41,154);

for i=1:G
    AMNE.Eq6.T7.T7GVC.FDI_GVC.V_fie_Y_DOE_E_SL(:,i)=AMNE.Eq6.T7.T7GVC.FDI_GVC.V_fie_Y_DOE_E(:,2*i);
end

AMNE.Eq6.T7.T7GVC.FDI_GVC.V_fie_Y_DOE_E_CL=sum(AMNE.Eq6.T7.T7GVC.FDI_GVC.V_fie_Y_DOE_E_SL,1)';
...
AMNE.Eq6.T8.T8GVC.FDI_GVC.V_fie_Y_FIE_L=sum(Results.GVC.FDI_GVC.V_fie_Y_FIE_L,2);
AMNE.Eq6.T8.T8GVC.FDI_GVC.V_fie_Y_FIE_L=reshape(AMNE.Eq6.T8.T8GVC.FDI_GVC.V_fie_Y_FIE_L,41,154);

for i=1:G
    AMNE.Eq6.T8.T8GVC.FDI_GVC.V_fie_Y_FIE_L_SL(:,i)=AMNE.Eq6.T8.T8GVC.FDI_GVC.V_fie_Y_FIE_L(:,2*i);
end

AMNE.Eq6.T8.T8GVC.FDI_GVC.V_fie_Y_FIE_L_CL=sum(AMNE.Eq6.T8.T8GVC.FDI_GVC.V_fie_Y_FIE_L_SL,1)';



Results.GVC.FDI_GVC.V_fie_Y_FIE_E=Term1.T9;
AMNE.Eq6.T9.T9GVC.FDI_GVC.V_fie_Y_FIE_E=sum(Results.GVC.FDI_GVC.V_fie_Y_FIE_E,2);
AMNE.Eq6.T9.T9GVC.FDI_GVC.V_fie_Y_FIE_E=reshape(AMNE.Eq6.T9.T9GVC.FDI_GVC.V_fie_Y_FIE_E,41,154);

for i=1:G
    AMNE.Eq6.T9.T9GVC.FDI_GVC.V_fie_Y_FIE_E_SL(:,i)=AMNE.Eq6.T9.T9GVC.FDI_GVC.V_fie_Y_FIE_E(:,2*i);
end

AMNE.Eq6.T9.T9GVC.FDI_GVC.V_fie_Y_FIE_E_CL=sum(AMNE.Eq6.T9.T9GVC.FDI_GVC.V_fie_Y_FIE_E_SL,1)';



%%
%? ?𝑟𝑎𝑑𝑒 𝑎𝑛𝑑 𝐹𝐷𝐼 𝑟𝑒𝑙𝑎𝑡𝑒𝑑 𝐺𝑉𝐶
...
% Term1.T10=diag(V_D)*LM*A_E*B*diag(Y_F); 
Results.GVC.Trade_FDI_GVC.V_doe_Y_FIE_L_F=Term1.T10; % Term1.T10=diag(V_D)*LM*A_E*B*diag(Y_F); vv

% sum every 41 block
for i=1:G
    AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F(:,i)=sum(Results.GVC.Trade_FDI_GVC.V_doe_Y_FIE_L_F(:,82*i-40:82*i),2);
end

for i=1:G
    AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F_D_SL(41*i-40:41*i,:)=AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F(82*i-81:82*i-41,:);
    % AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F_F(41*i-40:41*i,:)=AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F(82*i-40:82*i,:);  
end


for i=1:G
    AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F_D_CL(i,:)=sum(AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F_D_SL(41*i-40:41*i,:),1);
    % AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F_F(41*i-40:41*i,:)=AMNE.Eq6.T10.T10Trade_FDI_GVC.V_doe_Y_FIE_L_F(82*i-40:82*i,:);  
end
...
% sum every 41 block
for i=1:G
    AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F(:,i)=sum(Results.GVC.Trade_FDI_GVC.V_fie_Y_DOE_L_F(:,82*i-81:82*i-41),2);
end

for i=1:G
    AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F_D_SL(41*i-40:41*i,:)=AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F(82*i-81:82*i-41,:);
    AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F_F_SL(41*i-40:41*i,:)=AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F(82*i-40:82*i,:);  
end


for i=1:G
    AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F_F_CL(i,:)=sum(AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F_F_SL(41*i-40:41*i,:),1);
    % AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F_F(41*i-40:41*i,:)=AMNE.Eq6.T11.T11Trade_FDI_GVC.V_doe_Y_FIE_L_F(82*i-40:82*i,:);  
end
...
% sum every 41 block
for i=1:G
    AMNE.Eq6.T12.T12Trade_FDI_GVC.V_fie_Y_FIE_L_F(:,i)=sum(Results.GVC.Trade_FDI_GVC.V_fie_Y_DOE_L_F(:,82*i-81:82*i-41),2);
end

for i=1:G
    AMNE.Eq6.T12.T12Trade_FDI_GVC.V_doe_Y_FIE_L_F_D_SL(41*i-40:41*i,:)=AMNE.Eq6.T12.T12Trade_FDI_GVC.V_fie_Y_FIE_L_F(82*i-81:82*i-41,:);
    AMNE.Eq6.T12.T12Trade_FDI_GVC.V_doe_Y_FIE_L_F_F_SL(41*i-40:41*i,:)=AMNE.Eq6.T12.T12Trade_FDI_GVC.V_fie_Y_FIE_L_F(82*i-40:82*i,:);  
end


for i=1:G
    AMNE.Eq6.T12.T12Trade_FDI_GVC.V_doe_Y_FIE_L_F_F_CL(i,:)=sum(AMNE.Eq6.T12.T12Trade_FDI_GVC.V_doe_Y_FIE_L_F_F_SL(41*i-40:41*i,:),1);
    % AMNE.Eq6.T12.T12Trade_FDI_GVC.V_doe_Y_FIE_L_F_F(41*i-40:41*i,:)=AMNE.Eq6.T12.T12Trade_FDI_GVC.V_doe_Y_FIE_L_F(82*i-40:82*i,:);  
end
