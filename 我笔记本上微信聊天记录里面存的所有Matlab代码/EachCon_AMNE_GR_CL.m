...
%function [output1, output2]=EachCon_AMNE_GR_CL(V1)
function [output2]=EachCon_AMNE_GR_CL(V1)
% V1=results2.CL.Z_Exports  
% num=G;
C.ARGD = V1(:,1);
C.ARGF = V1(:,2);
C.AUSD = V1(:,3);
...
GR2.G1.NEU_OECD = [C.AUSD+C.CAND+C.CHLD+C.COLD+C.CRID+C.ISLD+C.ISRD+ C.JPND+C.KORD+ C.MEXD+ C.NZLD+C.NORD+C.TURD+C.GBRD C.AUSF+C.CANF+C.CHLF+C.COLF+ C.CRIF+ C.ISLF+ C.ISRF+ C.JPNF+ C.KORF+ C.MEXF+C.NZLF+ C.NORF+ C.TURF + C.GBRF]; % 14 
GR2.G1.US = [C.USAD C.USAF]; % USA
GR2.G1.NEUC = [C.CHED+C.UKRD C.CHEF+C.UKRF]; % Non-EU countries

%%
%ONO-OECD 32 COUNTRIES
GR2.G1.China=[C.CHND+C.HKGD+C.TWND C.CHNF+C.HKGF+C.TWNF];%3
GR2.G1.Russia = [C.RUSD C.RUSF]; % 1
GR2.G1.ASEAN = [C.BRND+C.KHMD+C.IDND+C.MYSD+C.MMRD+C.PHLD+C.SGPD+C.THAD+C.VNMD+ C.LAOD C.BRNF+C.KHMF+ C.IDNF+C.MYSF+C.MMRF+C.PHLF+C.SGPF+C.THAF+C.VNMF+C.LAOF]; % 10
GR2.G1.India = [C.INDD C.INDF]; % 1
GR2.G1.Africa = [C.CMRD+C.CIVD+C.EGYD+C.MARD+C.NGAD+C.SEND+C.ZAFD+C.TUND+C.JORD+ C.SAUD C.CMRF+C.CIVF+C.EGYF+C.MARF+C.NGAF+C.SENF+ C.ZAFF+C.TUNF+C.JORF+ C.SAUF]; % 10 Africa and Middle East
GR2.G1.NAMR = [C.ARGD+C.BRAD+C.PERD C.ARGF+C.BRAF+ C.PERF]; % 3 North America
GR2.G1.ONOECD = [C.BGDD+C.PAKD+C.KAZD C.BGDF+C.PAKF+C.KAZF]; % 3 Other Non-OECD
...
GR2.G2.NEU_OECD = [R.AUSD+R.CAND+R.CHLD+R.COLD+R.CRID+R.ISLD+R.ISRD+ R.JPND+R.KORD+ R.MEXD+ R.NZLD+R.NORD+R.TURD+R.GBRD 
    R.AUSF+R.CANF+R.CHLF+R.COLF+ R.CRIF+ R.ISLF+ R.ISRF+ R.JPNF+ R.KORF+ R.MEXF+R.NZLF+ R.NORF+ R.TURF + R.GBRF]; % 14 


GR2.G2.US = [R.USAD 
    R.USAF]; % USA
GR2.G2.NEUC = [R.CHED+R.UKRD 
    R.CHEF+ R.UKRF]; % Non-EU countries
...
GR2.G2.India = [R.INDD 
    R.INDF]; % 1
GR2.G2.Africa = [R.CMRD+R.CIVD+R.EGYD+R.MARD+R.NGAD+R.SEND+R.ZAFD+R.TUND+R.JORD+ R.SAUD 
    R.CMRF+R.CIVF+R.EGYF+R.MARF+R.NGAF+R.SENF+ R.ZAFF+R.TUNF+R.JORF+ R.SAUF]; % 10 Africa and Middle East
GR2.G2.NAMR = [R.ARGD+R.BRAD+R.PERD
    R.ARGF+R.BRAF+ R.PERF]; % 3 North America
GR2.G2.ONOECD = [R.BGDD+R.PAKD+R.KAZD
    R.BGDF+R.PAKF+R.KAZF]; % 3 Other Non-OECD
