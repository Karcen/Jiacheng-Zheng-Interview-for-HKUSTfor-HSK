...
m=163;%部门数量
mn=m*n;
E0=xlsread('Intensity.csv','intensity','B2:B7988'); %行向量或列向
% L0=xlsread('L.csv','L','B2:KUF7988');%完整的里昂惕夫逆矩阵
F0=xlsread('Finaldemand.csv','finaldemand','B2:MF7988');%完整的最终需求矩阵
A0=xlsread('a_matrix.csv','a_matrix','B2:KUF7988');%完整的中间投入矩阵
...
% L1=xlsread('L.csv','L','B2:KUF7988');%完整的里昂惕夫逆矩阵
...
Fa=F0; % Fa 是去掉对角线地区的最终需矩阵
for i=1:n
    Fa((i-1)*m+1:i*m,i)=0;
end
Fa=diag(sum(Fa,2)); %每个地区终产品出口 mn对角矩阵
...
E0=Etemp;
% 变量1：得到E是n*mn矩阵，每行对应列的位置是该国家的直接强度
...
for k = 1:n
    ...
    Fa=Q.*D; % Fa mn*n �???终需求矩�???
    Faa=diag(sum(Fa,2))*ones(mn,n)-Fa;  %每一列代表全球final demand减去该国final demand
    ...
    Zaa=diag(sum(Z,2))*ones(mn,n)-Ztemp;%每一列代表全球中间需求减去该国中间需�???
...
xlswrite("EXPORT_SDA.xlsx",E_N,'EN');
xlswrite("EXPORT_SDA.xlsx",E_W,'EW');
xlswrite("EXPORT_SDA.xlsx",T_N,'TN');
xlswrite("EXPORT_SDA.xlsx",T_W,'TW');
xlswrite("EXPORT_SDA.xlsx",H_N,'HN');
xlswrite("EXPORT_SDA.xlsx",H_W,'HW');
xlswrite("EXPORT_SDA.xlsx",D_N,'DN');
xlswrite("EXPORT_SDA.xlsx",D_W,'DW');
xlswrite("EXPORT_SDA.xlsx",Q_N,'QN');
xlswrite("EXPORT_SDA.xlsx",Q_W,'QW');

xlswrite("EXPORT_SDA.xlsx",B,'B');

xlswrite("EXPORT_SDA.xlsx",EEE0,'2010_export');
xlswrite("EXPORT_SDA.xlsx",EEE1,'2015_export');
